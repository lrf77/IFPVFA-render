...
st.sidebar.markdown('<p style="font-size:10px">3. If you want to see specific metadata from the sources, check the corresponding checkboxes under "Select Metadata".</p>', unsafe_allow_html=True)
st.sidebar.markdown('<p style="font-size:10px">4. Select the model you want to use from the dropdown menu. The options are "gpt-4" and "gpt-3.5-turbo".</p>', unsafe_allow_html=True)
st.sidebar.markdown('<p style="font-size:10px">5. Click the "Submit" button to get the answer to your question.</p>', unsafe_allow_html=True)
st.sidebar.markdown('<p style="font-size:10px">6. The answer will appear in the main area of the app. If you checked the "Show Sources" checkbox, the sources will appear below the answer.</p>', unsafe_allow_html=True)
st.sidebar.markdown('<p style="font-size:10px">7. If you want to ask another question, simply enter it in the text input field and click "Submit" again.</p>', unsafe_allow_html=True)
st.sidebar.markdown('<p style="font-size:10px">8. To navigate to a different page, select the page from the dropdown menu in the sidebar.</p>', unsafe_allow_html=True)
...
# Based on the user's selection, run the appropriate script
if page == "Home":
        ...
        if show_sources:
            ...
            for i, source in enumerate(llm_response["source_documents"]):
                ...
                with st.expander(f'Source {i+1}: {title}'):
                    ...
                    #producer = source.metadata.get("/Producer", "N/A") if show_producer else None
                    ...
                    #if producer: st.write(f"Producer: {producer}")
